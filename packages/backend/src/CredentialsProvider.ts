...
export class CredentialsProvider {
   ...
   */
  public async registerUser(username: string, plaintextPassword: string): Promise<ICredentialDocument | null> {
  ...
  // 3) Insert credentials document
  const newUser: ICredentialDocument = {
    _id: username,
    username,
    password: hashed,
  };
